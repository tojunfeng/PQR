...
class Generate:
    ...
    def __init__(self, data_dir):
        cache_dir = "./generated_query_llama_mg_8b"
...
class GenerateByAPI(Generate):
    ...
    def _generate(self, passage_list: List[str]) -> List[list[str]]:
        ...
        for i, passage in enumerate(passage_list):
            ...
            steps = [1, 2, 4]
            for step in steps:
                step_size = math.ceil(len(sentences) / (step+3))
                step_size = max(1, step_size)
                window_size = step_size * 4
                for step_i in range(step):
                    start = (step_i * step_size) % len(sentences)
                    end = min(start + window_size, len(sentences))
                    start = max(0, end - window_size)
                    seg_passage = " ".join(sentences[start:end])
                    prompt = self.prompt_fn(seg_passage)
